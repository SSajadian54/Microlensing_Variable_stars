...
///=================================================
   ...
   param=fopen("./files/param_all.txt","w");
   fclose(param); 
   ...
   fclose(tems);  
   double t, u, Rstar, Lstar[nf], A[nf], ro_star, Mag[3],del[nf],del_mag[2];
   double xlens, ylens,Tstar;
   int j; 
...
for(int icon=0; icon<1; ++icon){ 
   ...
   Mag[0]= vbb->ESPLMag2(u,s.ro_star0);
   Mag[1]= vbb->ESPLMag2(u,  ro_star );
   ...
   if(Tstar<0.0 or Rstar<0.0 or Tstar==0.0 or Rstar==0.0 or Tstar>12000.0 or Tstar<21.0 or Rstar>10000.0 or j<0 or 
   ...
   cout<<"****************************************************************"<<endl;
   cout<<"time(tE): "<<(t-l.t0)/l.tE<<endl;
   cout<<"u: "<<u<<"\t Ro_star: "<<ro_star<<"\t u/rho_star: "<<u/ro_star<<endl;
   cout<<"xlens: "<<xlens<<"\t ylens: "<<ylens<<"\t Tstar: "<<Tstar<<endl;
   cout<<"Magnificationp[0]: "<<Mag[0]<<"\t magnification[1]: "<<A[1]<<"\t del_mag: "<<del_mag<<endl; 
   int uue;  cin>>uue;}
   fprintf(magni,"%.4lf  %.4lf  %.3lf  %.4lf  %.4lf  %.5lf  %.5lf  %.5lf %.5lf  %.5lf  %.6lf  %.6lf  %.6lf  %.6lf  %.7lf  %.7lf  %.7lf %.7lf  %.7lf %.7lf %.5lf %.7lf\n",
   (t-l.t0)/l.tstar,Rstar,Tstar,Mag[0],Mag[1],Mag[2],A[1],A[2],A[3],A[4],Lstar[1],Lstar[2],Lstar[3],Lstar[4],
     del[1],del[2],del[3],del[4],del_mag[0],del_mag[1],u,ro_star);}//22
   fclose(magni);


   FILE* param;
   param=fopen("./files/param_all.txt","a+");
   fprintf(param,"%d  %d  %.5lf  %.5lf  %.5lf  %.5lf  %.5lf  %.5lf  %.5lf  %.5lf  %.7lf   %.5lf  %.5lf  %d  %d  %d  %.5lf  %.5lf  %.5lf    %.5lf   %.5lf  %.5lf  %.8lf  %.5lf  %.5lf  %.5lf  %.5lf  %.5lf  %.5lf  %.5lf  %.5lf  %.5lf  %.5lf  %.5lf  %.5lf  %d\n",
   ...
   p.peri,p.phir*RA,p.phit*RA,p.deltaR,p.deltaT,p.tp,icon);//36
   fclose(param);
...
void Func_lens(source & s , lens & l)
{
 ...
 l.u0=randR(0.0,20.0)*s.ro_star0;///*s.ro_star0;
    l.tstar= fabs(l.tE*s.ro_star0);///days
 l.pt1=-100.0*l.tstar;// l.tstar;///days
 l.pt2=+100.0*l.tstar;//l.tstar;///days
...
///XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
void Func_puls(lens & l , pulsing & p){
p.peri= randR(0.1,10.0);
    p.phit= randR(0.0,359.0)*pi/180.0;
    p.phir= p.phit-pi/2.0;
p.deltaT= randR(100.0,400.0); 
p.deltaR= randR(0.05,0.25); 
